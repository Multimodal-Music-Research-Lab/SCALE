...
from scale.data.label2id import (
...
import os
import random
from .sheetsagemsa_adapter import SheetSageMSAAdapter
    ...
    def __init__(
        ...
        for dataset_abstract_item in dataset_abstracts:
            ...
            if adapter is not None:
                ...
                if adapter == "SheetSageMSAAdapter":
                    self.adapter_obj[dataset_abstract_item["internal_tmp_id"]] = (
                        SheetSageMSAAdapter(**dataset_abstract_item, hparams=self.hparams)
                    )
    ...
    def init_segments(
        ...
        with open(label_path) as f:
            for line in f:
                ...
                starts = [x[0] for x in labels]
                names = [x[1] for x in labels]
                self.time_datas[hybrid_id] = starts
                self.time_datas[hybrid_id] = list(
                    map(float, self.time_datas[hybrid_id])
                )
                self.label_datas[hybrid_id] = [
                    -1 if name == "end" else self.label_to_id[name]
                    for name in names
                ]
    ...
    def __getitem__(self, idx):
        try:
            ...
            if adapter_str is not None:
                ...
                if adapter_str == "SheetSageMSAAdapter":
