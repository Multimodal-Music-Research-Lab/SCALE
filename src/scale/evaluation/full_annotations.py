...
import mir_eval
import pandas as pd
from scale.data.custom_types import MsaInfo
from scale.data.label2id import LABEL_TO_ID
from scale.data.msa_info_utils import load_msa_info
...
def compute_results_robust(
    ann_inter, est_inter, ann_labels, est_labels, bins, est_file, weight
):
    """Run canonical MSAF metrics while filtering zero-length aligned intervals.

    Boundary metrics retain the original, untrimmed interval convention.  Only
    the label-clustering inputs are aligned to the reference support, matching
    ``msaf.eval.compute_results`` except for removal of zero-duration rows.
    """
    ...
    adjusted_ann, adjusted_ann_labels = mir_eval.util.adjust_intervals(
        ann_inter, list(ann_labels)
    )
    adjusted_est, adjusted_est_labels = mir_eval.util.adjust_intervals(
        est_inter,
        list(est_labels),
        t_min=0.0,
        t_max=adjusted_ann.max(),
    )
    ann_valid = adjusted_ann[:, 1] > adjusted_ann[:, 0]
    est_valid = adjusted_est[:, 1] > adjusted_est[:, 0]
    adjusted_ann = adjusted_ann[ann_valid]
    adjusted_est = adjusted_est[est_valid]
    adjusted_ann_labels = np.asarray(adjusted_ann_labels)[ann_valid]
    adjusted_est_labels = np.asarray(adjusted_est_labels)[est_valid]
    result["PWP"], result["PWR"], result["PWF"] = mir_eval.segment.pairwise(
        adjusted_ann,
        adjusted_ann_labels,
        adjusted_est,
        adjusted_est_labels,
    )
    result["So"], result["Su"], result["Sf"] = mir_eval.segment.nce(
        adjusted_ann,
        adjusted_ann_labels,
        adjusted_est,
        adjusted_est_labels,
